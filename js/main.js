...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.querySelectorAll('.timeline-step').forEach(step => {
    step.addEventListener('click', () => {
      const detailId = step.dataset.detail;
      if (!detailId) return;
      const detail = document.getElementById(detailId);
      if (!detail) return;
      // Toggle
      const isShown = detail.classList.contains('show');
      // Hide all in this panel
  ...
  const chatPromptClose = document.getElementById('chatPromptClose');
  const chatBtn = document.getElementById('chatBtn');
  chatPromptClose.addEventListener('click', () => { chatPrompt.style.display = 'none'; });
  chatBtn.addEventListener('click', () => {
    if (chatPrompt.style.display !== 'none') chatPrompt.style.display = 'none';
    window.open('https://wa.me/919958041888?text=Hi%20EduQuest%2C%20I%20have%20a%20question%20about%20admissions.', '_blank');
  });
  setTimeout(() => { chatPrompt.style.display = 'none'; }, 8000);

  /* ---- Calc card click ---- */
  document.querySelectorAll('.calc-card').forEach(card => {
    card.addEventListener('click', () => {
      window.open('https://test.eduquest.org.in/', '_blank');
    });
  });
});
